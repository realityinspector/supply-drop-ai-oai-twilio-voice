...
OPENAI_API_KEY = os.getenv('OPENAI_API_KEY')  # Add this to Replit Secrets
PORT = int(os.getenv('PORT', 8080))  # Replit typically uses port 8080
...
if not OPENAI_API_KEY:
    raise ValueError('Missing the OpenAI API key. Please add it to Replit Secrets.')
                ...
                async for message in websocket.iter_text():
                    ...
                    if data['event'] == 'media' and openai_ws.open:
                        ...
                        if logger:
                            ...
                            logger.debug(f"Audio payload size: {len(data['media']['payload'])}")
                ...
                async for openai_message in openai_ws:
                    ...
                    if logger:
                        if response['type'] in LOG_EVENT_TYPES:
                            ...
                            logger.debug(f"Full event payload: {json.dumps(response)}")
                            
                        if response['type'] == 'session.updated':
                            logger.info("Session updated successfully")
                            logger.debug(f"Session update payload: {json.dumps(response)}")
                    ...
                    if response['type'] == 'response.audio.delta' and response.get('delta'):
                        try:
                            ...
                            if logger:
                                ...
                                logger.debug(f"Audio response size: {len(audio_payload)}")
                        except Exception as e:
                            if logger:
